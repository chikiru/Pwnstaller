...
int bRpGhyQZic(ARCHIVE_STATUS *nWvEhHXrIlBF, char const * BYOuyVDyJjEGI, char  const * IDFkaONVuH);
int QCvSyDwTysgwoCO(ARCHIVE_STATUS *vwaEpVDGNO[]);
int JDFJnZJbzPWYDT(ARCHIVE_STATUS *RgmsRlXmpmr, int QmkMDhoYok, char *eGseisoYoaqPSZ[]);
int callSimpleEntryPoint(char *VlAlUewUv, int *hXVpkYrYurkGd);
void snUuCrtQx(ARCHIVE_STATUS *nJCRpkSsUEf);
int XHNXUGGqBqB(ARCHIVE_STATUS *koNjNJUZUqA);
void EIgfGSlBJZQIaS(void);
int GDYRVJXHADmy(ARCHIVE_STATUS *oMGGCGqRcvK, char const * qCSvhAkOjtF, char const * PwmTiaqascc);
int acJORotZvpUt(ARCHIVE_STATUS *jqcAnPovfrgFb);
int attachPython(ARCHIVE_STATUS *NHmCEyDiYbXm, int *TuUyoSlwXzCQco);
int OZgLWEexaNfGXph(ARCHIVE_STATUS *iCzyUwF);
int UTXErUXCMv(ARCHIVE_STATUS *LjdiZLNTbEkctw, int xYgZJtqAaoBXEg, char *ehALSHrHMQpGE[]);
int BTJAbjcAPPZD(ARCHIVE_STATUS *uDFyDJPxxqZ);
int CUqmSyIqeVQMOq(ARCHIVE_STATUS *NEEzpSKOYpDg);
int vxFgxNlecXiV(ARCHIVE_STATUS *gghjWylxuUTTYS);
TOC *getFirstTocEntry(ARCHIVE_STATUS *uZYfsAyPCGQB);
TOC *getNextTocEntry(ARCHIVE_STATUS *DlOXFr, TOC *tsfUrNYdGhFA);
void PyzGtZyZeeEN(const char *hDFVPn);
#endif
